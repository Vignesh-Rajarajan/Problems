...
public class MinObstaclesToReachEndII {
    ...
    public int minCost(int[][] grid) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int dir = 1; dir <= 4; dir++) {
                int newRow = row + dirs[dir - 1][0];
                int newCol = col + dirs[dir - 1][1];
                ...
                if (
                    ...
                    int newCost = cost + (dir != (grid[row][col]) ? 1 : 0);
    ...
    public int minCostOptimised(int[][] grid) {
        int numRows = grid.length, numCols = grid[0].length;
