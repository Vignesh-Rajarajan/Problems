...
public class FindTheCity {
     ...
     * so Dijkstras immediately pops into mind.
     * Dijkstras is a smallest path finding algorithm that works on graphs with positive weights.
    ...
    public int findTheCityFloydWarshal(int n, int[][] edges, int distanceThreshold) {
        ...
        for (int[] row : dis) {
            // the reason for 10001 is that if we put Integer.MAX_VALUE, we will get overflow while adding,
            // and we will get negative values which will mess up the comparison
